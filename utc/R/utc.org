    ...
    Crop_image_to_each_Shapefile_polygon <- function(shapefile.dsn,
        ...
        foreach (i = seq_along(polygons),
                 .packages = c("raster","rgeos")) %dopar% {
                     r <- image
                     poly.wBuff <- gBuffer(polygons[i],width = 15)
                     r <- crop(r, poly.wBuff)
...
#+BEGIN_SRC R
  ...
  getPixel.Class.and.Coords.Within.Polygon <- function(PixelFeatureDF,
      ...
      proj4string(PixelFeatureDF) <- wtm

      training.sp <- spTransform(training.sp,wtm)

      pts.in.poly <- sp::over(PixelFeatureDF,training.sp)
      PixelFeatureDF@data <- cbind(PixelFeatureDF@data, pts.in.poly)
      PixelFeatureDF <- PixelFeatureDF[which(complete.cases(pts.in.poly$class)),]
      PixelFeatureDF@data
  }
...
#+begin_src R
             Build.and.Save.models <- function(dir = dd.training.dir,
                ...
                dat <- readRDS(paste0(dir,"/",modelBuildingData)) %>%
                    as.data.frame() %>%
                    dplyr::select(-matches("image.name|segment|Id|segment.id|polygon.id|tile.name")) %>%
                    filter(complete.cases(.))
    ...
    if(n.subset != "none") {
          n.subset <- min(nrow(dat), n.subset)
          dat <- dat[sample(nrow(dat),n.subset),]
    }
                ...
                SVM_response <- makeLearner(id = "svm_resp", "classif.svm", predict.type = "response", fix.factors.prediction = TRUE)
                 ADA_prob <- makeLearner(id = "ada_prob", "classif.boosting", predict.type = "prob", fix.factors.prediction = TRUE)
               GBM_prob <- makeLearner(id = "gbm_prob", "classif.gbm", predict.type = "prob", fix.factors.prediction = TRUE)
                                         #      learner.list <- list(RF_prob = RF_prob, RF_response = RF_response, SVM_response = SVM_response)
                learner.list <- list(RF_prob = RF_prob, SVM_response = SVM_response, ADA_prob = ADA_prob,  GBM_prob = GBM_prob)
                                                    # Train Learners on Tasks, Make models
                                                    #         cl<-makeCluster(cores)
                                                    #         registerDoParallel(cl)

                models <- foreach(lnr = learner.list) %do% {
                    mod <- mlr::train(lnr, tsk)
                    mod
                    saveRDS(mod, file = paste0(models.dir,"/",location,image.name,"_",seg.p, "_",lnr$id,"_Untuned","_Subset-",n.subset,model.append, ".rds"))
                }
            }
...
#+begin_src R
  Build.and.Save.FeatureSelected.models <- function(dir = dd.training.dir,
      ...
      inner = makeResampleDesc("CV", iter = 10)
...
*** tuned
#+begin_src R
        Tune.Build.and.Save.models <- function(dir = dd.training.dir,
                                               modelBuildingData = ModelBuildingRDS,
                                               models.dir = dd.models.dir,
                                               image.name,
                                               location,
            ...
            dat <- readRDS(paste0(dir,"/",modelBuildingData)) %>%
                as.data.frame() %>%
                select(-image.name, -segment, -Id, -segment.id, -polygon.id, -tile.name, -id) %>%
                filter(complete.cases(.))
            ...
            rf.ps <- makeParamSet(makeIntegerParam("ntree", lower = 1L, upper = 500L),
                                  ...
                                  makeDiscreteParam("fw.abs", values = seq_len(getTaskNFeats(iris.task)))))
            ...
            ada.ps <- makeParamSet(makeIntegerParam("mfinal", lower = 1L, upper = 500),
                                   makeIntegerParam("minsplit", lower = 1L, upper = 50),
                                   makeDiscreteParam("fw.abs", values = seq_len(getTaskNFeats(iris.task)))))

            # shrinkage (learning rate) and complextity should be tuned in boosting. - these two parameters determine the number of trees
            ...
            ada.lrn =  makeTuneWrapper("classif.boosting", resampling = inner, par.set = ada.ps, control = ctrl, show.info = T)

            parallelStartSocket(n.cores)

                                                # outer loop for testing accuracy
            lrnrs = list(svm.lrn, rf.lrn, ada.lrn)
            outer = makeResampleDesc("CV", iters = 10L)
            res = benchmark(lrnrs, tsk, outer, measures = acc, show.info = FALSE)
            res
            parallelStop()
        }
#+end_src
...
"                           makeIntegerParam("minsplit", lower = 1L, upper = 50),
                           makeDiscreteParam("fw.abs", values = seq_len(getTaskNFeats(iris.task)))))"
Error in checkClass(x, classes, ordered) : object 'svm.ps' not found
Error in checkClass(x, classes, ordered) : object 'rf.ps' not found
Error in requirePackages(package, why = stri_paste("learner", id, sep = " "),  :
  For learner classif.boosting please install the following packages: adabag
Error in assertInt(x, na.ok, lower, upper, tol, .var.name = .var.name,  :
  object 'n.cores' not found
Error: object 'svm.lrn' not found
Error in isScalarValue(x) : object 'lrnrs' not found
standardGeneric for "res" defined from package "raster"

function (x)
standardGeneric("res")
<bytecode: 0x7f9a276e4790>
<environment: 0x7f9a276edeb0>
