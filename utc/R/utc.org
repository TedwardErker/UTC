...
#+PROPERTY: header-args:R :cache no :results output :exports both :tangle utc.R :session *R:utc:sumac*
...
#+begin_src R
       Build.and.Save.models <- function(dir = dd.training.dir,
          ...
          dat <- readRDS(paste0(dir,"/",modelBuildingData)) %>%
              as.data.frame() %>%
              dplyr::select(-image.name, -segment, -Id, -segment.id, -polygon.id, -tile.name) %>%
              filter(complete.cases(.))
...
function (x)
standardGeneric("res")
<bytecode: 0x7f8f2d64a990>
<environment: 0x7f8f2d6540b0>
