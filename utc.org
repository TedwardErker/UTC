...
#+begin_src R
  ...
  prob <- .10
    ...
    quant <- quantile(areas[,1],probs = prob)
...
#+results:
: Warning message:
: In dir.create(temp_i) : '../R_raster_temp/1' already exists
...
#+results:
: [1] "make tile output dir ../DD/ClassifiedUrbanAreas/1/m_4208831_se_16_1_20130619_w"
...
#+BEGIN_SRC R
  ...
  writeRaster(x = out, filename = paste0(urb.path,"/ClassifiedUrbanArea_",i,".tif"), overwrite = T, datatype = 'INT1U')
  ...
  writeRaster(x = out, filename = paste0(urb.path,"/rgbn_",i,".tif"), overwrite = T)
...
#+results:
: [1] "Wrote ClassifiedUrbanArea_1.tif"
: [1] "Wrote RGBN_1.tif"
