...
What do I want the summary plots to look like?

Start general, then show details.

**** Factors underconsideration
1) classifier
2) definition of "truth", what we compare to: Grids, Field Data,
   random points
3) Segmentation
4) which image (naip or spot)

each combination is a point.

segmentation are continuous variables
...
messing around
#+begin_src R
  ras <- raster(paste0(dd.accuracy.dir,"/ClassifiedTiles/madisonNAIP.mad.200m.3_Pixel_madisonNAIP_all_rf_prob.tif"))
#+end_src
