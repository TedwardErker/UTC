...
**** Make Feature and model building data frames, for Each Training Tile
...
compactness parameter is 3
['madisonNAIP.1_pca.tif']
average number of pixels per segment is 30.0
compactness parameter is 15
['madisonNAIP.1_pca.tif']
average number of pixels per segment is 30.0
compactness parameter is 90
['madisonNAIP.1_pca.tif']
...
#+begin_src R
  ...
  band.appendage = "_ratio.nir"
  window <- list(c(3,3), c(5,5))
  statistics = list("homogeneity", "contrast", "correlation")
  shift = list(c(0,1),c(1,0),c(1,1),c(-1,1))

  texture.params <- expand.grid(band.appendage = band.appendage,window = window, statistics = statistics, shift = shift, stringsAsFactors = F)


        make.feature.df(image.dir = dd.training.dir,
                        image.name = i,
                        band.names = c("blue","green","red","nir"),
                        ndvi = T,
                        ratio.bands = c("blue","green","red","nir"),
