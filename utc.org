...
5) [ ] Assess Accuracy Across State (Madison, Wausau, NE WI, perhaps
   Eau Claire and others)
6) [ ] Classify whole state
   1) [ ] add buffers to urban areas to allow for growth (10% diameter
      of urban area)
   2) [ ] relabel water, crops, and wetland
7) [ ] Merge classifications into giant raster
...
#+begin_src R
  ...
  sunPosition(
      when = seamlines_by_ua@data$date[1],
      lat = seamlines_by_ua@data$latitude[1],
      long = seamlines_by_ua@data$longitude[1])

  sunPosition(lat = seamlines_by_ua@data$latitude[1], long = seamlines_by_ua@data$longitude[1])
