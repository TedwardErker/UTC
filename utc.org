...
Madison Area and then Wisconsin.
...
#+begin_src R
#  segment.size <- c(rep(15,3), rep(20,3),rep(30,3),rep(45,3),rep(60,3),rep(100,3))
  segment.size <- c(rep(15,3), rep(20,3),rep(30,3),rep(45,3),rep(60,3))
  compactness <- round(segment.size * c(.3, .5, .6))
...
#+results:
:  Warning messages:
: 1: In as.POSIXct.POSIXlt(lt) : unknown timezone 'CDT'
: 2: In as.POSIXlt.POSIXct(ct) : unknown timezone 'CDT'
...
#+results:
: Warning messages:
: 1: In as.POSIXlt.POSIXct(x, tz) : unknown timezone 'CDT'
...
[1] "city-Appleton_month-7_hour-8_solElv-31_azimuth-88_NAIP_Training"
 ...
 [8] "city-Milwaukee_month-6_hour-10_solElv-51_azimuth-106_NAIP_Training"
 [9] "city-Milwaukee_month-6_hour-11_solElv-62_azimuth-126_NAIP_Training"
[10] "city-NewRichmond_month-8_hour-10_solElv-45_azimuth-113_NAIP_Training"
[11] "city-Racine_month-6_hour-9_solElv-46_azimuth-100_NAIP_Training"
[12] "city-Reedsburg_month-6_hour-12_solElv-70_azimuth-168_NAIP_Training"
[13] "city-Rhinelander_month-9_hour-10_solElv-38_azimuth-126_NAIP_Training"
[14] "city-Watertown_month-6_hour-8_solElv-37_azimuth-92_NAIP_Training"
[15] "city-Wausau_month-8_hour-9_solElv-39_azimuth-111_NAIP_Training"
#+end_example
...
#+results:
:  OGR data source with driver: ESRI Shapefile
: Source: "../WholeState_RD/Training_Regions", layer: "city-Milwaukee_month-6_hour-10_solElv-51_azimuth-106_NAIP_Training"
: with 46 features
: It has 2 fields
: [[1]]
: NULL
...
#+begin_src R
      ...
      out <- foreach(train.dir = train.dirs[8]) %do% {

          tile.names <- list.files(train.dir) %>%
              str_extract(., pattern = ".*[0-9]+.tif") %>%
              str_extract(., pattern = "t[0-9]+") %>%
              unique() %>%
              na.omit()
...
#+BEGIN_SRC R :results none
          ...
          model.building.dfs <-  foreach(train.dir = train.dirs[3:15],.packages = c("mlr","foreach","doParallel", "stringr", "raster","rgeos","dplyr","sp","tidyr")) %do% {

              featureDF.files <- list.files(train.dir) %>%
                  str_extract(., str_c("^_(",segmentation.pattern,")", feature.df.appendage,".rds$")) %>%
    #            str_extract(.,".*Pixel.*") %>%   #ADDED THIS TO DEBUG THE PIXEL FEATURE DF
                  na.omit()
...
#+begin_src R
  ...
  #  model.building.rdss <- model.building.rdss  %>% str_extract(.,".*Pixel.*") %>% na.omit  # THIS IS FOR DEBUGGING PIXEL FEATUER DF

          foreach(model.building.rds = model.building.rdss) %dopar% {
...
#+begin_src R
  ...
  out <-    foreach(segment.param.set = segment.param.sets, .packages = c("dplyr", "stringr")) %dopar% {
...
#+begin_src R
  ...
  df <- sample_n(full.df, size = 10000)
#df <- full.df
#+end_src

#+results:

#+begin_src R :exports results :results graphics :file ./figs/VisualizeModelBuildingDF_whole.state.png :width 1000 :height 700
  ggplot(data = df, aes(x = ratio.nir_mean, y = ratio.nir_stat.homogeneity_window.7_angle.0_mean,color = class)) +
      geom_point(alpha = .7) +
      facet_wrap(~solarElevation, ncol=5) +
      utc.col

  ## ggplot(data = df, aes(x = ratio.nir, y = ratio.nir_stat.homogeneity_window.3_angle.0, color = class)) +
  ##     geom_point(alpha = .1) +
...
#+begin_src R :exports results :results graphics :file ./figs/elevation_and_ratioNIR.png :width 700
  ## ggplot(data = df, aes(y = ratio.nir_stat.homogeneity_window.5_angle.0, x = factor(solarAzimuth), fill = class)) +
  ##     geom_boxplot(alpha = .8) +
  ##     utc.fill

  ggplot(data = df, aes(y = ratio.nir_mean, x = factor(solarAzimuth), fill = class)) +
      geom_boxplot(alpha = .8) +
      utc.fill
...
proj4string(ua_wi) <- proj4string(wi)
  ua_wi <- crop(ua_wi, wi)
    ...
    pts <- spsample(ua_wi, n=4000, type = "random")
#+end_src

#+results:
#+begin_example
../WholeState_DD/Accuracy
average number of pixels per segment is 45.0
compactness parameter is 27
P.1112_pca.tif
OGR data source with driver: ESRI Shapefile
Source: "../RD_US_UrbanAreasShapefile", layer: "cb_2013_us_ua10_500k"
with 3601 features
...
***** load points, reproject to wtm and remove non grass, impervious, and tree points
...
#+BEGIN_SRC R :results none
    ...
    ###
    ###           I am selecting the 3rd model, 30-15 rf, just to get an initial classificaotn
    ###

  foreach(model = models[3]) %do% {
...
It has 2 fields
 ../WholeState_DD/Accuracy
average number of pixels per segment is 15.0
compactness parameter is 4
P.1175_pca.tif
../WholeState_DD/Accuracy
average number of pixels per segment is 15.0
compactness parameter is 8
P.1175_pca.tif
../WholeState_DD/Accuracy
average number of pixels per segment is 15.0
compactness parameter is 9
P.1175_pca.tif
../WholeState_DD/Accuracy
average number of pixels per segment is 20.0
compactness parameter is 6
P.1175_pca.tif
../WholeState_DD/Accuracy
average number of pixels per segment is 20.0
...
compactness parameter is 36
P.1174_pca.tif
[1] "2016-10-07 12:43:20 CDT"
 Error in function_list[[k]](value) :
  could not find function "ascii.nowarn.print"
Warning messages:
...
3: In if (class(x[, j]) != class(dd[, j])) { :
  the condition has length > 1 and only the first element will be used
[1] "org_babel_R_eoe"
[1] "2016-10-07 09:17:51 CDT"
[1] "2016-10-07 09:14:09 CDT"
 |   | image          | target.cover | model   | seg.params | tuned   | mean.overall.error |
|---+----------------+--------------+---------+------------+---------+--------------------|
| 1 | WholeStateNAIP | all          | rf_prob | N-30_C-15  | Untuned | 0.21               |
[1] "2016-10-06 20:46:42 CDT"
 |   | image          | target.cover | model   | seg.params | tuned   | mean.overall.error |
|---+----------------+--------------+---------+------------+---------+--------------------|
| 1 | WholeStateNAIP | all          | rf_prob | N-30_C-15  | Untuned | 0.10               |
[1] "2016-10-06 20:46:05 CDT"
 |   | image          | target.cover | model   | seg.params | tuned   | mean.overall.error |
|---+----------------+--------------+---------+------------+---------+--------------------|
| 1 | WholeStateNAIP | all          | rf_prob | N-30_C-15  | Untuned | 0.10               |
[1] "2016-10-06 20:40:17 CDT"
 |   | image          | target.cover | model   | seg.params | tuned   | mean.overall.error |
|---+----------------+--------------+---------+------------+---------+--------------------|
| 1 | WholeStateNAIP | all          | rf_prob | N-30_C-15  | Untuned | 0.10               |
...
#+begin_src R
      e.df <-   error.df %>%
        filter(seg.params == "N-30_C-15", model == "rf_prob") %>%
...
|-----+---------------+---------+----------------------------------------------|
| 1   | 1.00          | 11.00   | Eau Claire, WI                               |
| 2   | 1.00          | 13.00   | Waupaca, WI                                  |
| 3   | 1.00          | 14.00   | Appleton, WI                                 |
| 4   | 1.00          | 103.00  | Shawano, WI                                  |
| 5   | 1.00          | 104.00  | Wisconsin Rapids, WI                         |
| 6   | 1.00          | 106.00  | Lake Geneva, WI                              |
| 7   | 1.00          | 110.00  | Madison, WI                                  |
| 8   | 1.00          | 113.00  | Fond du Lac, WI                              |
| 9   | 1.00          | 114.00  | Madison, WI                                  |
| 10  | 1.00          | 117.00  | Milwaukee, WI                                |
| 11  | 1.00          | 124.00  | Appleton, WI                                 |
| 12  | 1.00          | 125.00  | Milwaukee, WI                                |
| 13  | 1.00          | 143.00  | Milwaukee, WI                                |
| 14  | 1.00          | 158.00  | Clintonville, WI                             |
| 15  | 1.00          | 166.00  | Shawano, WI                                  |
| 16  | 1.00          | 179.00  | Madison, WI                                  |
| 17  | 1.00          | 181.00  | Fort Atkinson, WI                            |
| 18  | 1.00          | 189.00  | Eau Claire, WI                               |
| 19  | 1.00          | 190.00  | Janesville, WI                               |
| 20  | 1.00          | 191.00  | Appleton, WI                                 |
| 21  | 1.00          | 192.00  | Madison, WI                                  |
| 22  | 1.00          | 201.00  | Round Lake Beach--McHenry--Grayslake, IL--WI |
| 23  | 1.00          | 208.00  | Milwaukee, WI                                |
| 24  | 1.00          | 212.00  | Stevens Point, WI                            |
| 25  | 1.00          | 214.00  | Milwaukee, WI                                |
| 26  | 1.00          | 216.00  | Milwaukee, WI                                |
| 27  | 1.00          | 217.00  | Milwaukee, WI                                |
| 28  | 1.00          | 1004.00 | Oshkosh, WI                                  |
| 29  | 1.00          | 1009.00 | Milwaukee, WI                                |
| 30  | 1.00          | 1014.00 | Milwaukee, WI                                |
| 31  | 1.00          | 1021.00 | Rhinelander, WI                              |
| 32  | 1.00          | 1031.00 | Stevens Point, WI                            |
| 33  | 1.00          | 1039.00 | Chetek, WI                                   |
| 34  | 1.00          | 1051.00 | Racine, WI                                   |
| 35  | 1.00          | 1053.00 | Tomah, WI                                    |
| 36  | 1.00          | 1058.00 | Kenosha, WI--IL                              |
| 37  | 1.00          | 1063.00 | Milwaukee, WI                                |
| 38  | 1.00          | 1067.00 | Prairie du Sac--Sauk City, WI                |
| 39  | 1.00          | 1070.00 | Madison, WI                                  |
| 40  | 1.00          | 1072.00 | Hudson, WI--MN                               |
| 41  | 1.00          | 1099.00 | Milwaukee, WI                                |
| 42  | 1.00          | 1104.00 | Wausau, WI                                   |
| 234 | 1.00          | 2169.00 | Kenosha, WI--IL                              |
| 235 | 1.00          | 2170.00 | Oshkosh, WI                                  |
| 236 | 1.00          | 2174.00 | Sturgeon Bay, WI                             |
| 237 | 1.00          | 2175.00 | Kenosha, WI--IL                              |
| 238 | 1.00          | 2179.00 | Baraboo, WI                                  |
| 239 | 1.00          | 2181.00 | Kenosha, WI--IL                              |
| 240 | 1.00          | 2186.00 | Osceola, WI                                  |
| 241 | 1.00          | 2198.00 | Appleton, WI                                 |
...
|----+----------------------------------------------+--------+-------+-------|
| 1  | Abbotsford, WI                               | 1.00   | 0.00  | 9.00  |
| 2  | Algoma, WI                                   | 2.00   | 0.00  | 10.00 |
| 3  | Ashland, WI                                  | 4.00   | 0.00  | 9.75  |
| 4  | Beloit, WI--IL                               | 11.00  | 0.00  | 10.00 |
| 5  | Black River Falls, WI                        | 3.00   | 0.00  | 11.00 |
| 6  | Cambridge, WI                                | 1.00   | 0.00  | 10.00 |
| 7  | Chilton, WI                                  | 1.00   | 0.00  | 9.00  |
| 8  | Columbus, WI                                 | 1.00   | 0.00  | 10.00 |
| 9  | Dodgeville, WI                               | 1.00   | 0.00  | 13.00 |
| 10 | East Troy, WI                                | 1.00   | 0.00  | 8.00  |
| 11 | Edgerton, WI                                 | 2.00   | 0.00  | 10.00 |
| 12 | Ellsworth, WI                                | 1.00   | 0.00  | 8.00  |
| 13 | Hayward, WI                                  | 1.00   | 0.00  | 10.00 |
| 14 | Iron Mountain--Kingsford, MI--WI             | 2.00   | 0.00  | 11.00 |
| 15 | Johnson Creek, WI                            | 1.00   | 0.00  | 8.00  |
| 16 | Kiel--New Holstein, WI                       | 1.00   | 0.00  | 9.00  |
| 17 | Lake Mills, WI                               | 3.00   | 0.00  | 9.00  |
| 18 | Lancaster, WI                                | 2.00   | 0.00  | 8.00  |
| 19 | Mauston, WI                                  | 1.00   | 0.00  | 11.00 |
| 20 | Medford, WI                                  | 1.00   | 0.00  | 9.00  |
| 21 | Menomonie, WI                                | 5.00   | 0.00  | 9.00  |
| 22 | Merrill, WI                                  | 2.00   | 0.00  | 9.00  |
| 23 | Mount Horeb, WI                              | 2.00   | 0.00  | 9.00  |
| 24 | New London, WI                               | 2.00   | 0.00  | 8.00  |
| 25 | New Richmond, WI                             | 1.00   | 0.00  | 8.00  |
| 26 | Oconto Falls, WI                             | 1.00   | 0.00  | 9.00  |
| 27 | Oconto, WI                                   | 2.00   | 0.00  | 10.00 |
| 28 | Omro, WI                                     | 1.00   | 0.00  | 8.00  |
| 29 | Oregon, WI                                   | 4.00   | 0.00  | 10.00 |
| 30 | Platteville, WI                              | 2.00   | 0.00  | 10.50 |
| 31 | Plymouth, WI                                 | 1.00   | 0.00  | 10.00 |
| 32 | Portage, WI                                  | 4.00   | 0.00  | 10.00 |
| 33 | Reedsburg, WI                                | 3.00   | 0.00  | 11.33 |
| 34 | Sparta, WI                                   | 5.00   | 0.00  | 12.00 |
| 76 | Stevens Point, WI                            | 15.00  | 0.33  | 9.00  |
| 77 | Walworth, WI                                 | 3.00   | 0.33  | 9.00  |
| 78 | Appleton, WI                                 | 49.00  | 0.35  | 8.08  |
| 79 | Wisconsin Rapids, WI                         | 17.00  | 0.35  | 9.47  |
| 80 | Tomahawk, WI                                 | 5.00   | 0.40  | 9.00  |
| 81 | Round Lake Beach--McHenry--Grayslake, IL--WI | 7.00   | 0.43  | 10.86 |
| 82 | Antigo, WI                                   | 2.00   | 0.50  | 9.00  |
| 83 | Berlin, WI                                   | 2.00   | 0.50  | 9.00  |
| 84 | Chetek, WI                                   | 2.00   | 0.50  | 8.50  |
| 85 | Delavan, WI                                  | 2.00   | 0.50  | 9.00  |
| 86 | Ironwood, MI--WI                             | 2.00   | 0.50  | 8.00  |
| 87 | Prairie du Sac--Sauk City, WI                | 6.00   | 0.50  | 9.00  |
| 88 | Shawano, WI                                  | 6.00   | 0.50  | 8.00  |
| 89 | Sheboygan, WI                                | 16.00  | 0.50  | 9.00  |
| 90 | Somerset, WI                                 | 2.00   | 0.50  | 9.00  |
| 91 | Union Grove, WI                              | 4.00   | 0.50  | 10.00 |
| 92 | Rice Lake, WI                                | 3.00   | 0.67  | 9.00  |
| 93 | Osceola, WI                                  | 4.00   | 0.75  | 9.00  |
| 94 | Clintonville, WI                             | 2.00   | 1.00  | 9.00  |
| 95 | Kewaunee, WI                                 | 1.00   | 1.00  | 10.00 |
| 96 | Minneapolis--St. Paul, MN--WI                | 1.00   | 1.00  | 9.00  |
| 97 | Sturgeon Bay, WI                             | 2.00   | 1.00  | 10.00 |
#+end_example

***** Percent of wrong points by hour
#+begin_src R
  n.pts <-   error.df %>%
      filter(seg.params == "N-30_C-15", model == "rf_prob") %>%
                                          #         select(overall.error, point,NAME10) %>%
      ...
      summarize(n.pts = n(),
                error = sum(overall.error)/n()) %>%
      mutate(sd_error = sqrt(error*(1-error)/n.pts)) %>%
      arrange(error) %>%
      ascii.nowarn.print
...
#+results:
:  |   | hour  | n.pts  | error | sd_error |
: |---+-------+--------+-------+----------|
: | 1 | 13.00 | 1.00   | 0.00  | 0.00     |
: | 2 | 12.00 | 25.00  | 0.12  | 0.06     |
: | 3 | 10.00 | 435.00 | 0.16  | 0.02     |
: | 4 | 11.00 | 224.00 | 0.23  | 0.03     |
: | 5 | 8.00  | 181.00 | 0.24  | 0.03     |
: | 6 | 9.00  | 279.00 | 0.26  | 0.03     |

***** Percent of wrong points by month
#+begin_src R
  n.pts <-   error.df %>%
      filter(seg.params == "N-30_C-15", model == "rf_prob") %>%
                                          #         select(overall.error, point,NAME10) %>%
...
#+results:
:  |   | month | n.pts  | error | sd_error |
: |---+-------+--------+-------+----------|
: | 1 | 6.00  | 578.00 | 0.21  | 0.02     |
: | 2 | 7.00  | 490.00 | 0.21  | 0.02     |
: | 3 | 8.00  | 69.00  | 0.23  | 0.05     |
: | 4 | 9.00  | 8.00   | 0.25  | 0.15     |

***** Percent of wrong points by sun angle  I SHOULD JUST GRAPH THESE
#+begin_src R

        sunPos <- sunPosition(
            when = as.POSIXct(as.character(error.df$date)),
            lat = error.df$latitude,
            long = error.df$longitude)

        error.df.wAngle <- cbind(error.df, sunPos) %>%
          mutate(solarElevationBinned = round_any(elevation, 5),
                 solarAzimuthBinned = round_any(azimuthJ, 5))

  error.df.wAngle %>%
    group_by(solarElevationBinned) %>%
    summarize(n.pts = n(),
              error = sum(overall.error)/n()) %>%
  mutate(sd_error = sqrt(error*(1-error)/n.pts)) %>%
      arrange(solarElevationBinned) %>%
      ascii.nowarn.print
...
#+results:
#+begin_example
 |   | solarElevationBinned | n.pts  | error | sd_error |
|---+----------------------+--------+-------+----------|
| 1 | 30.00                | 129.00 | 0.28  | 0.04     |
| 2 | 35.00                | 86.00  | 0.17  | 0.04     |
| 3 | 40.00                | 128.00 | 0.25  | 0.04     |
| 4 | 45.00                | 125.00 | 0.28  | 0.04     |
| 5 | 50.00                | 266.00 | 0.15  | 0.02     |
| 6 | 55.00                | 162.00 | 0.18  | 0.03     |
| 7 | 60.00                | 158.00 | 0.23  | 0.03     |
| 8 | 65.00                | 81.00  | 0.23  | 0.05     |
| 9 | 70.00                | 10.00  | 0.00  | 0.00     |
#+end_example

#+begin_src R
  az <- error.df.wAngle %>%
    group_by(solarAzimuthBinned) %>%
    summarize(n.pts = n(),
              error = sum(overall.error)/n()) %>%
  mutate(sd_error = sqrt(error*(1-error)/n.pts)) %>%
      arrange(solarAzimuthBinned)
...
#+results:
#+begin_example
 |    | solarAzimuthBinned | n.pts  | error | sd_error |
|----+--------------------+--------+-------+----------|
| 1  | 85.00              | 59.00  | 0.22  | 0.05     |
| 2  | 90.00              | 125.00 | 0.26  | 0.04     |
| 3  | 95.00              | 57.00  | 0.26  | 0.06     |
| 4  | 100.00             | 164.00 | 0.26  | 0.03     |
| 5  | 105.00             | 264.00 | 0.15  | 0.02     |
| 6  | 110.00             | 52.00  | 0.27  | 0.06     |
| 7  | 115.00             | 161.00 | 0.17  | 0.03     |
| 8  | 120.00             | 6.00   | 0.17  | 0.15     |
| 9  | 125.00             | 164.00 | 0.23  | 0.03     |
| 10 | 130.00             | 21.00  | 0.24  | 0.09     |
| 11 | 135.00             | 2.00   | 0.00  | 0.00     |
| 12 | 140.00             | 49.00  | 0.24  | 0.06     |
| 13 | 145.00             | 4.00   | 0.25  | 0.22     |
| 14 | 150.00             | 1.00   | 0.00  | 0.00     |
| 15 | 155.00             | 6.00   | 0.17  | 0.15     |
| 16 | 170.00             | 9.00   | 0.00  | 0.00     |
| 17 | 180.00             | 1.00   | 0.00  | 0.00     |
#+end_example

#+begin_src R :exports results :results graphics :file ./figs/Error.DF.wAngle.png

  ggplot(az, aes(x = solarAzimuthBinned, y = error, size = n.pts)) + geom_point()
#+end_src

#+results:
[[file:./figs/Error.DF.wAngle.png]]
