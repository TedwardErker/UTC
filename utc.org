...
Error in ogrInfo(dsn = dsn, layer = layer, encoding = encoding, use_iconv = use_iconv,  :
  Cannot open file
Error in subset(ua, grepl(".*WI", as.character(ua@data$NAME10))) :
  object 'ua' not found
Error in .class1(object) : object 'ua_wi' not found
 Error in { :
  task 1 failed - "object of type 'closure' is not subsettable"
Error: object 'lat.mid' not found
 Error in { :
  task 1 failed - "object of type 'closure' is not subsettable"
Error: object 'long.mid' not found
#+end_example
...
#+begin_src R
  ...
  full.df <- readRDS(ModelBuildingRDSs[16])
#+end_src
...
#+begin_src R :exports results :results graphics :file ./figs/elevation_and_ratioNIR.png :width 1000
  ...
  ggplot(data = df, aes(y = ratio.nir, x = factor(solarAzimuth), fill = class)) +
      geom_boxplot(alpha = .8) +
      utc.fill
...
#Adjusting Segment Params for speed

  segment.params <- data.frame(compactness = 15, segment.size = 30)
